...
class SysLanguages extends CWidget
{
    ...
    public $default_template = 'products';
    ...
    {
        $limit = 3;
        $products = ExtProduct::model()->findAllByAttributes(array('is_new' => 1),array('order' => 'time_created DESC', 'limit' => $limit));
        $productArr = array();

        foreach($products as $index => $objProducts)
        {
            $newsArr[$index] = $objProducts;
        }
        ...
        $this->render($template,array('products' => $productArr));
    }
}